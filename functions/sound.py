...
def play(track, immediately_play=True):
    ...
    if track is not None:
            ...
            try:
                ...
                if track.artists[0].bio is None:
                    track.artists[0].populate(
                        application.api.get_artist_info(
                            track.artists[0].id
                        )
                    )
        ...
        if stream is not application.stream and application.stream is not None:
            if config.sound['pyglet']:
                ...
                Thread(
                    target=fadeout, args=[application.stream]
                ).start()
        if immediately_play:
            stream.play(True)
    ...
    if application.track is not track and not isinstance(track, db.URLStream):
        Thread(
            target=application.frame.update_lyrics,
            args=[track]
        ).start()
...
def get_next(remove=True):
    """Get the next track which should be played. If remove == True, delete the
    track from the queue if that's where it came from."""
    if hasattr(
        application.frame,
        'repeat_track'
    ) and application.frame.repeat_track.IsChecked():
    ...
    if application.frame is not None and application.frame.queue:
        ...
        if hasattr(
            application.frame,
            'shuffle'
        ) and application.frame.shuffle.IsChecked():
            if remove:
                if len(
                    application.frame.played
                ) == len(
                    application.frame.results
                ):
                    application.frame.played.clear()
                t = choice(
                    [x for x in application.frame.results if
                        x not in application.frame.played]
                )
                ...
                return None
        try:
            t = application.frame.results[application.frame.results.index(
                application.track
            ) + 1]
        except (IndexError, AttributeError):  # We're at the end.
            if application.frame is not None and \
               application.frame.results and hasattr(
                   application.frame,
                   'repeat_all'
               ) and application.frame.repeat_all.IsChecked():
...
def get_previous():
    ...
    if hasattr(
        ...
        return application.track
    try:
        return application.frame.results[
            application.frame.results.index(
                application.track
            ) - 1
        ]
...
def seek(amount):
    ...
    application.stream.set_position(
        max(
            0,
            min(
                application.stream.get_length(),
                application.stream.get_position() + amount
            )
        )
    )
...
def set_output_device(name):
    ...
    if device != application.output.device:
        ...
        # Calling .stop() on a stream after it's device has been dropped causes
        # a traceback. Let's not do that.
