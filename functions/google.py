...
class PlaylistAction(object):
    ...
    def complete(self):
        ...
        dlg = wx.SingleChoiceDialog(
            ...
            self.message,
            self.title,
            [x.name for x in playlists]
        )
        if dlg.ShowModal() == wx.ID_OK:
            return self.callback(
                playlists[dlg.GetSelection()],
                *self.args,
                **self.kwargs
            )
        dlg.Destroy()
...
def playlist_action(message, title, callback, *args, **kwargs):
    """call callback with the resulting playlist as the first argument,
    ...
    application.frame.playlist_action = PlaylistAction(
        message,
        title,
        callback,
        args,
    ...
    Thread(
        target=localise_playlists,
        args=[application.frame.playlist_action]
    ).start()
...
def add_to_playlist(playlist, *tracks):
    ...
    for pos, id in enumerate(
        application.api.add_songs_to_playlist(
            playlist.id,
            [x.id for x in tracks]
        )
    ):
        session.add(
            PlaylistEntry(
...
def remove_from_playlist(*entries):
    ...
    try:
        application.api.remove_entries_from_playlist(
...
def load_artist_top_tracks(artist):
    ...
    wx.CallAfter(
        ...
        showing='%s top tracks' % artist.name
    )


def artist_action(artists, callback, *args, **kwargs):
    """select an artist from artists and call
    callback(artist, *kwargs, **kwargs)."""
    do_login()
...
def create_station(field, data, name=''):
    ...
    if not name:
        name = wx.GetTextFromUser(
            'Enter the name for the new radio station',
    ...
    def f2(albums):
        ...
        if not albums:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                Thread(
                    target=callback,
                    args=[albums[dlg.GetSelection()]['albumId'], *args],
                    kwargs=kwargs
                ).start()
    Thread(
        target=f1,
