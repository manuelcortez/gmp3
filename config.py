...
from simpleconf.validators import Integer, Boolean, Float, Option as \
     OptionValidator
...
class Config(Section):
    ...
    class sound(Section):
        """Sound Configuration."""
        title = 'Sound'
        fadeout_threshold = Option(
            0,
            title='Remaining Samples Before &Fadeout',
            validator=Integer(
                min=0
            )
        )
        fadeout_amount = Option(
            1.0,
            title='Fadeout &Amount',
            validator=Float(
                max=1.0,
                min=0.00001
            )
        ...
        pyglet = Option(
            False,
            title='Output Sound Using &Pyglet',
            validator=PygletValidator
        )
