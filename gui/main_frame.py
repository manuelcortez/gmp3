...
class MainFrame(wx.Frame):
    ...
    def play_pause(self, event):
        ...
        if self.cast_device is not None:
            m = self.cast_device.media_controller
            if m.is_playing:
                m.pause()
                logger.info('Pause device %r.', self.cast_device)
            elif m.is_paused:
                m.play()
                logger.info('Play device %r.', self.cast_device)
            else:
                logger.warning('Inactive device %r.', self.cast_device)
        elif application.stream is not None:
